...
export default function AppShell() {
  ...
  useEffect(()=>{let cancelled=false;const media=window.matchMedia('(prefers-color-scheme: dark)');let selected='system';const sync=()=>applyTheme(selected);(async()=>{const {data}=await requireSupabase().from('user_preferences').select('theme').eq('organization_id',auth.organization.id).eq('user_id',auth.user.id).maybeSingle();if(cancelled)return;selected=data?.theme||'system';sync();})();media.addEventListener('change',sync);return()=>{cancelled=true;media.removeEventListener('change',sync);};},[auth.organization.id,auth.user.id]);
