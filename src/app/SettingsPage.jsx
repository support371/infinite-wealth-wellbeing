...
export default function SettingsPage() {
  ...
  useEffect(()=>{(async()=>{const {data,error:queryError}=await requireSupabase().from('user_preferences').select('*').eq('organization_id',auth.organization.id).eq('user_id',auth.user.id).maybeSingle();if(queryError)setError(queryError.message);if(data)setForm({theme:data.theme,locale:data.locale,...data.communication});})();},[auth.organization.id,auth.user.id]);
