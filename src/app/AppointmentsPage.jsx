...
import { AlertTriangle, CalendarDays, CheckCircle2, Clock3, MapPin, Plus, RefreshCcw, ShieldCheck, UserRound, X } from 'lucide-react';
...
export default function AppointmentsPage() {
  const auth = useAuth();
  const [state, setState] = useState({ loading: true, error: '', appointments: [], participants: [], calendarConnected: false });
  ...
  const load = useCallback(async () => {
    ...
    const [appointmentsResult, participantsResult, connectionsResult] = await Promise.all([
    ...
    const error = appointmentsResult.error || participantsResult.error || connectionsResult.error;
    if (error) {
    ...
    setState({ loading: false, error: '', appointments: appointmentsResult.data || [], participants, calendarConnected: (connectionsResult.data || []).length > 0 });
  ...
  return <section className="workspace-page appointments-page">
    <header className="workspace-heading"><div><span className="workspace-eyebrow">SECURE SCHEDULING</span><h1>Appointments</h1><p>Request, confirm and track advisor or practitioner sessions with organization-validated participants.</p></div><button className="app-button primary" onClick={() => setShowForm(true)} disabled={!memberOptions.length || !hostOptions.length}><Plus size={16}/> Request appointment</button></header>
    ...
    {notice && <div className={`form-alert ${notice.includes('saved') || notice.includes('marked') ? 'success' : 'error'}`}>{notice.includes('saved') || notice.includes('marked') ? <CheckCircle2/> : <AlertTriangle/>}{notice}</div>}
