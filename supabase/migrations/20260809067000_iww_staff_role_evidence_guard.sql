-- Correct role-management evidence semantics.
-- System bootstrap has no user actor; no-op revocations are recorded as unchanged attempts.
...
begin
  ...
  insert into public.iww_audit_events (
    ...
    'user_role',
    p_user_id::text,
    jsonb_build_object('targetUserId', p_user_id, 'role', 'admin', 'systemBootstrap', true)
  );

  return jsonb_build_object('userId', p_user_id, 'role', 'admin', 'active', true, 'changed', true);
end;
$$;
...
begin
  ...
  insert into public.iww_audit_events (
    ...
    jsonb_build_object('role', p_role, 'active', p_active, 'changed', v_changed)
  );
